...
if(!window.SpaJs)
{
    ...
    window.SpaJs = function(spa_name)
    {
        ...
        spajs.goToURL = function(url, title)
        {
            let opt = {}
            opt.menuId = url
            ...
            try{
                ...
                $.when(res).fail(() => {
                    ...
                    {
                        console.error("Рекурсивная перезагрузка", newUrl)
                    }
                })
            }catch (exception) {
                console.trace("exception", exception)
        ...
        spajs.openMenuFromUrl = function(event_state, opt = {})
        {
            ...
            if(event_state)
            {
                ...
                opt.menuId = event_state.slice(window.location.origin.length+1)
            }
            else
            {
                // Если menu_url не задан то используем window.location.hash
                opt.menuId = window.location.pathname.slice(1)
            }
        ...
        spajs.sortMenu = function(targetBlock)
        {
            ...
            {
                a = parseInt(jQuery(a).attr("data-index"));
                if(isNaN(a))
                {
                    return 1;
                }

                b = parseInt(jQuery(b).attr("data-index"));
                if(isNaN(b))
                {
                    return -1;
                }
        ...
        spajs.loadServerPage = function(url, after_push_state)
        {
            ...
            jQuery.ajax({
                ...
                success: function(res)
                {
                    ...
                    // debugger;
                    tabSignal.disconnectAllFromSignal("loading.completed.spajs")
                    tabSignal.disconnectAllFromSignal("loading.completed.libs")
                    tabSignal.disconnectAllFromSignal("loading.completed.pages")
                    tabSignal.disconnectAllFromSignal("loading.completed.chat")
                    tabSignal.disconnectAllFromSignal("loading.completed.js")
                    tabSignal.disconnectAllFromSignal("loading.completed")
                    ...
                    // обработать ответ и вставить только тело в страницу
                    ...
                    tabSignal.emit("loading.newPage.once")
                    tabSignal.disconnectAllFromSignal("loading.newPage.once")
                    // кассно былобы вызывать оnload и отписывать от него всех слушателей

                    // вынести в конфиг класс или список классов
                    $(".spa-link").on('click', function()
                    {
                        spajs.goToURL(this.href);
                        return false;
                    })
